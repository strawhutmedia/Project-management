...
export const api = {
  ...
  // Persistent per-episode carousel deck (auto-generated by the
  // upload-and-go pipeline; regenerable on demand).
  episodeCarouselDeck: (songId: string) =>
    request<{
      deck: {
        id: string
        status: 'generating' | 'generated' | 'failed'
        error: string | null
        deck: { slides: ApiCarouselDeckSlide[]; asset_requests: ApiCarouselAssetRequest[] } | null
        input_tokens: number
        output_tokens: number
        created_at: string
        updated_at: string
      } | null
      preset: { key: string; displayName: string; logo: { name1: string; name2: string; weight?: 'black' | 'bold' }; palette: { primary: string; secondary: string; bg: string; ink: string; inkDim: string } } | null
    }>(`/api/carousel/decks/${songId}`),
  generateEpisodeCarouselDeck: (songId: string) =>
    request<{ ok: true; status: 'generating' }>(`/api/carousel/decks/${songId}/generate`, { method: 'POST' }),
